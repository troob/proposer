...
def generate_prop_table_data(available_prop_dicts, desired_order=[], joint_sheet_name='Joints', rare_prev_val_players={}, prints_on=False):
    print('\n===Generate Prop Table Data===\n')
    print('Input: available_prop_dicts before = [{player: james harden, ...}, ...')# = ' + str(available_prop_dicts))
    ...
    prop_tables = [rare_prev_dicts, available_prop_dicts]
    sheet_names = ['Rare', 'All']
...
def generate_all_cur_conds_lists(all_cur_conds_dicts, all_game_player_cur_conds, all_players_abbrevs, season_years, part):
    ...
    all_players_gp_conds = determiner.determine_all_current_gp_conds(all_game_player_cur_conds, all_players_abbrevs)[1]
...
def generate_conditions_order(all_cur_conds_dicts, all_game_player_cur_conds, all_players_abbrevs, season_years, part):
    ...
    # for game players conds we just want overall for all yrs bc too many subconditions to show on this page
    gp_conds = determiner.determine_all_current_gp_conds(all_game_player_cur_conds, all_players_abbrevs)[0]
...
def generate_rare_prev_cond(rare_prev_val_players, player):

    # add most rare
    rare_prev_cond = 'No'

    for rare_cat, rare_cat_players in rare_prev_val_players.items():
        for rare_player in rare_cat_players:
            if rare_player == player:
...
def generate_low_sample_sizes(player_cur_conds_list, player_stat_dict, season_years, season_part, single_conds):
...
def generate_all_true_prob_dicts(all_true_probs_dict, all_players_teams={}, all_players_playtimes={}, all_cur_conds_dicts={}, all_gp_cur_conds={}, all_cur_conds_lists={}, all_last_vals={}, all_player_stat_dicts={}, game_teams=[], rosters={}, rare_prev_val_players={}, cur_yr='', season_years=[], season_part='', single_conds=[], stats_of_interest=[]):
    ...
    for player, player_stat_probs_dict in all_true_probs_dict.items():
        ...
        low_sample_size_data = generate_low_sample_sizes(player_cur_conds_list, player_stat_dict, season_years, season_part, single_conds)
        ...
        for stat_name, stat_probs_dict in player_stat_probs_dict.items():
            ...
            if stat_name in stats_of_interest:
                ...
                rare_prev = generate_rare_prev_cond(rare_prev_val_players, player)
                ...
                for val, val_probs_dict in stat_probs_dict.items():
                    ...
                    # #     if player_team
                    # stat_val_probs_dict['game'] = game_num
                    stat_val_probs_dict = generate_stat_val_probs_cond_refs(game_num, prev_val, playtime, confidence, player_current_conditions, player_gp_conds, stat_val_probs_dict, cond_low_sample_sizes, gp_low_sample_sizes, rare_prev)
...
def generate_game_players_condition_mean_prob(team_condition, player_team, opp_team, team_gp_conds, team_gp_conds_weights_dict, val_probs_dict, player_stat_dict, all_players_abbrevs, all_players_cur_avg_playtimes, cur_yr, season_years, part, stat, single_conds, prints_on, player_name):
    if prints_on:
    ...
    for cond in team_gp_conds.keys():
        ...
        if game_player in all_players_cur_avg_playtimes.keys():
            cond_weight = all_players_cur_avg_playtimes[game_player]#generate_gp_cond_weight(teammate_cur_season_log, teammate_gp_cur_team, cond)
        
            # get weighted avg bt all season yrs, like gen all conds weights?
            # no just get this yr bc minutes change drastically each yr
            condition_mean_prob = generate_condition_mean_prob(cond, val_probs_dict, player_stat_dict, season_years, part, stat, single_conds, prints_on)
    ...
    for cond_key, cond in multi_conds.items():
        ...
        cond_weight = multi_cond_weights[cond_key]
    ...
    # === Version 1 ===
    # group each team part as sub cond and then weight each player in subcond
    ...
    #         condition_mean_prob = generate_condition_mean_prob(condition, val_probs_dict, player_stat_dict, season_years, part, stat)
    ...
    #         if sum_weights > 0:
    #             team_part_true_prob = round_half_up(sum_weighted_probs / sum_weights,2)
    #         else:
    #             print('Warning: denom = sum_weights = 0 bc no samples for condition!')
    ...
    #     if sum_weights > 0:
    #         gp_condition_mean_prob = round_half_up(sum_weighted_probs / sum_weights,2)
    #     else:
    #         print('Warning: denom = sum_weights = 0 bc no samples for condition!')
...
def generate_condition_mean_prob(condition, val_probs_dict, player_stat_dict, season_years, part, stat_name, single_conds=[], prints_on=False):
    if prints_on:
    ...
    if condition not in single_conds:
        ...
        if len(probs) > 0:
            ...
            for p_idx in range(len(probs)):
                ...
                s_n = determiner.determine_sample_size(player_stat_dict, cur_conds, stat_name) # for a given yr
...
def generate_all_conditions_mean_probs(player_team, opp_team, prev_val, val_probs_dict, player_current_conditions, all_gp_conds, all_gp_conds_weights, player_stat_dict, all_players_abbrevs, all_players_cur_avg_playtimes, cur_yr, season_years, part, stat, player='player', val='val', single_conds=[], prints_on=False):
    if prints_on:
    ...
    for team_condition, team_gp_conds in all_gp_conds.items():
        ...
        #all_conditions_mean_probs[condition] = generate_condition_mean_prob(condition, val_probs_dict, season_years, part, player_stat_dict)
        condition_mean_prob = generate_game_players_condition_mean_prob(team_condition, player_team, opp_team, team_gp_conds, team_gp_conds_weights, val_probs_dict, player_stat_dict, all_players_abbrevs, all_players_cur_avg_playtimes, cur_yr, season_years, part, stat, single_conds, prints_on, player)
...
def generate_true_prob(player_team, opp_team, prev_val, prev_val_weight, val_probs_dict, player_current_conditions, all_conditions_weights_dict, all_gp_conds, all_gp_conds_weights, player_stat_dict, all_players_abbrevs, all_players_cur_avg_playtimes, cur_yr, season_years, part, player='player', stat='stat', val='val', single_conds=[], prints_on=False):
    if prints_on:
    ...
    # all_conditions_mean_probs = {c1:p1,...}
    all_conditions_mean_probs = generate_all_conditions_mean_probs(player_team, opp_team, prev_val, val_probs_dict, player_current_conditions, all_gp_conds, all_gp_conds_weights, player_stat_dict, all_players_abbrevs, all_players_cur_avg_playtimes, cur_yr, season_years, part, stat, player, val, single_conds, prints_on) # for the stat described in val probs dict
...
def generate_game_players_cond_sample_weight(cond_key, lineup_team, team_gp_cur_conds, player_stat_dict, part, player='', prints_on=False):
    if prints_on:
...
def generate_condition_sample_weight(player_stat_dict, cond_key, cond_val, single_conds, part, player='', stat=''):
...
def generate_all_conditions_weights(player_current_conditions, all_gp_conds, player_prev_vals, player_stat_dict, all_players_abbrevs, part, player='', player_team='', single_conds=[], prints_on=False):
    ...
    for cond_key, cond_val in player_current_conditions.items():
        ...
        if cond_key != 'opp team':
            condition_sample_weight = generate_condition_sample_weight(player_stat_dict, cond_key, cond_val, single_conds, part, player)
...
def generate_rare_prev_val_players(all_true_probs_dict, all_prev_vals, game_teams, all_players_teams, cur_yr, rosters):#, all_players_season_logs):
    print('\n===Generate Rare Prev Val Players===\n')
    print('Input: all_true_probs_dict = {player: {stat: {val: {true prob:prob, condition1:prob1, ... = {\'nikola jokic\': {\'pts\': {1: {\'all 2024 regular prob\': 0.0, ..., \'A Gordon PF, J Murray PG,... starters 2024 regular prob\': 0.0, ..., \'true prob\': 1.0}')
                ...
                for val, val_true_probs_dict in stat_true_probs_dict.items():
                    ...
                    if true_prob < rare_prob:
                        ...
                        if rare:
                            ...
                            prev_prob *= 100
                            rare_prev_val = (game_num, player_abbrev, stat.upper(), likely_val, prev_stat_val, num_rare, prev_prob, total_prob) #player + ', ' + stat + ', ' + str(likely_val) + ', ' + str(prev_stat_val)
                ...
                for val, val_true_probs_dict in stat_true_probs_dict.items():
                    ...
                    if true_prob < 1 - rare_prob:
                        ...
                        if rare:
                            ...
                            total_prob = round_half_up(total_prob * 100, 2)
                            true_prob *= 100
                            rare_prev_val = (game_num, player_abbrev, stat.upper(), likely_under, prev_stat_val, num_rare, true_prob, total_prob) #player + ', ' + stat + ', ' + str(likely_val) + ', ' + str(prev_stat_val)
...
def generate_all_true_probs_dict(all_stat_probs_dict, all_player_stat_dicts, all_players_abbrevs={}, all_players_teams={}, rosters={}, all_cur_conds_dicts={}, all_game_player_cur_conds={}, all_prev_vals={}, all_players_cur_avg_playtimes={}, game_teams=[], season_years=[], cur_yr='', stats_of_interest=[], single_conds=[], prints_on=False):
    if prints_on:
    ...
    for player, player_probs_dict in all_true_probs_dict.items():
        ...
        all_conditions_weights_data = generate_all_conditions_weights(player_current_conditions, all_gp_conds, player_prev_vals, player_stat_dict, all_players_abbrevs, part, player, player_cur_team, single_conds, prints_on)
        ...
        for stat, stat_probs_dict in player_probs_dict.items():
            ...
            if stat in stats_of_interest:
                ...
                prev_val_weight = generate_prev_val_weight(prev_val, player_stat_dict, part, player, stat, single_conds)
                ...
                for val, val_probs_dict in stat_probs_dict.items():
                    ...
                    val_probs_dict['true prob'] = generate_true_prob(player_cur_team, opp_team, prev_val, prev_val_weight, val_probs_dict, player_current_conditions, all_conditions_weights, all_gp_conds, all_gp_conds_weights, player_stat_dict, all_players_abbrevs, all_players_cur_avg_playtimes, cur_yr, season_years, part, player, stat, val, single_conds, prints_on)
...
# player_teams = {year:team:gp}
def generate_player_current_conditions(player, game_teams, player_teams, all_lineups={}, player_cur_season_log={}, init_game_ids_dict={}, all_teams_schedules={}, all_cur_games_info={}, player_abbrev='', cur_yr='', rosters={}, cur_date='', read_new_game_ids=True, todays_games_date_obj=datetime.today()):
    ...
    #print('Input: all_teams_schedules = {team: {field idx:{\'0\':field name, game num:field val, ... = {"phx": {"0": {"0": "DATE", "1": "Tue, Oct 24", "2": "Thu, Oct 26", ...')
    ...
    for player in players:
        ...
        if player in all_players_teams.keys():
            ...
            if not determiner.determine_dnp_player(player_teams, cur_yr, player):
                ...
                player_cur_conds_data = generate_player_current_conditions(player, game_teams, player_teams, all_lineups, player_cur_season_log, init_game_ids_dict, all_teams_schedules, all_cur_games_info, cur_yr=cur_yr, rosters=rosters)
...
# and use out conds
def generate_player_playtime(player_name, player_team, player_gp_conds, player_stat_dict, player_season_logs, all_players_season_logs, all_players_teams, all_players_abbrevs, all_players_cur_avg_playtimes, after_injury_players, irreg_play_times, cur_yr, season_part, prints_on):
    if prints_on:
    ...
    if player_name in irreg_play_times.keys():
        ...
        if len(player_gp_conds) > 0:
            ...
            for cond in teammate_gp_conds:
                ...
                if teammate in all_players_cur_avg_playtimes.keys():
                    ...
                    if teammate == player_name or cond_key == 'out':
                        ...
                        teammate_weight = all_players_cur_avg_playtimes[teammate]#generate_gp_cond_weight(teammate_cur_season_log, teammate_gp_cur_team, cond)
                        
                        
                        sample_size = determiner.determine_sample_size(player_stat_dict, cur_conds)
                        cond_weight = 0
                        cond_playtime = None
                        if sample_size > 1:
                            sample_weight = math.log(sample_size)
                            cond_weight = round_half_up(teammate_weight * sample_weight, 2)
            ...
            for cond_key, cond in multi_conds.items():
                ...
                if sample_size > 1:
                    sample_weight = math.log(sample_size)
                    cond_weight = round_half_up(teammates_weight * sample_weight, 2)
...
def generate_player_all_stats_dicts(player_name, player_game_log, opponent, player_teams, season_year, todays_games_date_obj, all_box_scores, year_games_info, player_teammates, all_seasons_stats_dicts, season_part, player_position, rosters={}, cur_team='', cur_yr='', gp_cur_team='', season_years=[], stats_of_interest=[]):
...
def generate_all_players_props(settings={}, players_names=[], game_teams=[], teams_current_rosters={}, all_teams=[], todays_games_date_obj=datetime.today()):
    ...
    if find_players == True:
        ...
        all_box_scores = reader.read_all_box_scores(all_players_season_logs, all_players_teams, season_years, season_part, current_year_str, read_new_game_ids)#, season_year) # go thru players in all_players_season_logs to get game ids
    ...
    for player_name in players_names:
        ...
        player_playtime = generate_player_playtime(player_name, player_team, player_gp_conds, player_stat_dict, player_season_logs, all_players_season_logs, all_players_teams, all_players_abbrevs, all_players_cur_avg_playtimes, after_injury_players, irreg_play_times, current_year_str, season_part, prints_on=True)
    ...
    rare_prev_val_players = generate_rare_prev_val_players(all_true_probs_dict, all_prev_vals, game_teams, all_players_teams, current_year_str, teams_current_rosters)


    # flatten nested dicts into one level and list them
    # all_stat_prob_dicts = [{player:player, stat:stat, val:val, conditions prob:prob,...},...]
    # add warnings to row in table
    all_true_prob_dicts = generate_all_true_prob_dicts(all_true_probs_dict, all_players_teams, all_players_playtimes, all_cur_conds_dicts, all_game_player_cur_conds, all_cur_conds_lists, all_last_vals, all_player_stat_dicts, game_teams, teams_current_rosters, rare_prev_val_players, current_year_str, season_years, season_part, single_conds, stats_of_interest)
    ...
    # 'warning', 
    desired_order.extend(['rare prev', 'cond warning', 'gp warning', 'confidence', 'playtime', 'prev'])
    ...
    prop_table_data = generate_prop_table_data(available_prop_dicts, desired_order, joint_sheet_name, rare_prev_val_players, prints_on)
